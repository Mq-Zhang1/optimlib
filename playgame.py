...
from Adversarial import Adversial
...
class GameRunner:
    ...
    def loop(self,depth,plot):
        ...
        while True:
            ...
            if plot == 1 and step>=100:
                game_over = 1
            if game_over:
                if Iplot == True:
                    Total_score.append(loop_score)
                    loop_score=[]
                    #print(loop_score)
                self.auto = False
                Iplot = False
            ...
            for event in pygame.event.get():
                ...
                if event.type == QUIT:
                    ...
                    for i in range(len(Total_score)):
                        ite = [j+1 for j in range(len(Total_score[i]))] # data length
                        plt.plot(ite,Total_score[i],color[i],label ='line '+str(i))
                        print("The final score for line {}:".format(i), Total_score[i][-1])
                    plt.legend()
            ...
            if self.auto and not game_over:
                if step ==100: step = 0
                Iplot = True
                ai = Adversial(self.game.get_state(),0,MOVES,0,search_depth=depth)
            ...
            if direction != None:
                ...
                step+=1
                loop_score.append(self.game.score)
            self.print_matrix()
            if game_over:
...
parser.add_argument('--depth','-d', dest="depth", type = int, default = 3, help='the depth of the tree')
...
if __name__ == '__main__':
    ...
    ''' update following code to plot the score iteration'''
    Total_score=[]
    game = GameRunner()
    game.loop(args.depth,args.plot)
